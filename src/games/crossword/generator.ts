/**
 * Daily-crossword puzzle generator.
 *
 * Pure and deterministic for a given numeric seed (typically from
 * `getDailySeed()`): picks a shape, generates a black-square mask
 * procedurally during the fill process (rather than sampling a static
 * library), runs the fill solver until a valid grid emerges, then attaches
 * one clue per entry from the on-disk clue bank.
 *
 * Procedural masks are produced by laying down 180°-rotationally-symmetric
 * black-cell pairs in seed-shuffled order, validating min-entry-length and
 * connectivity after each placement, and stopping when a per-shape target
 * black-cell count is reached. This gives more visual variety (especially
 * on the larger 7×N shapes) and packs in noticeably more black squares
 * than the previous static-pattern sampling.
 *
 * The clue bank is static JSON co-located in `./data/` so it ships with the
 * client bundle — no runtime fetching, no server route. The bank is
 * generated by `scripts/generate-clue-bank.mjs`. A small library of vetted
 * static patterns is kept as a *fallback* for cases where the procedural
 * search exhausts its budget without producing a fillable mask.
 */
...
const patternLib = patternsData as Partial<PatternLibrary>;
...
function tryShape(shape: ShapeKey, rng: () => number): Puzzle | null {
  ...
  // Last-resort fallback: any pre-vetted static patterns the repo happens
  // to ship for this shape. (New shapes may have none — that's fine, we
  // just skip.) These are tried as fixed masks, no adaptive growth.
  const libMasks = patternLib[shape] ?? [];
  for (const black of shuffle([...libMasks], rng)) {
    const filled = fillGrid(rows, cols, black, bankByLen, rng, {
      stepBudget: FILL_BUDGET,
    });
    if (!filled) continue;
    const puzzle = buildPuzzle(shape, rows, cols, black, filled, rng);
    if (puzzle) return puzzle;
  }
